...
==========================
Extraktion von Bild-Embeddings via CLIP (ViT-Large-Patch14).
...
def main():
    ...
    print(f"Cropped-Makro-Ordner:    {CROPPED_DIR.resolve()}")
    print("-" * 60)
    ...
    # CLIP laden (Einmalig für alle Ordner)
    print(f"Lade CLIP Vision-Modell: {CLIP_MODEL_ID}...")
    try:
        processor = CLIPProcessor.from_pretrained(CLIP_MODEL_ID, token=HF_TOKEN)
        model = CLIPVisionModel.from_pretrained(CLIP_MODEL_ID, device_map="auto", token=HF_TOKEN)
        print(f"CLIP Vision erfolgreich geladen auf Gerät: {model.device}\n")
    except Exception as e:
        print(f"Fehler beim Laden von CLIP: {e}")
        return
    ...
    for model_dir in model_dirs:
        ...
        if n_images == 0:
            print(f"Ordner '{t2i_model}' ist leer. Überspringe.")
            continue

        extracted_data = []
        
        # Schleife über ALLE physisch gefundenen Bilder dieses Modells
        for img_path in tqdm(img_paths, desc=f"CLIP [{t2i_model}]"):
            ...
            try:
                # Bild laden und Vektor berechnen
        ...
        if extracted_data:
            embeddings_output = OUTPUT_DIR / f"clip_embeddings_{t2i_model}.pkl"
            ...
            print(f"Keine Embeddings generiert für Ordner {t2i_model}.")

    print("\n" + "="*60)
    print("🎉 ABSOLUTER MAKRO-SCAN BEENDET! Alle pkl-Dateien wurden erzeugt.")
    print("="*60)
